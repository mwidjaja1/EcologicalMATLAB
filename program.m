%% -- Matthew Widjaja: Ecological Networks --------------------------------
% This code does knock outs, node sweeps, & slave node manipulations for
% Ecological Networking Model created by Jason Shulman's team at Stockton
% College. This works in conjunction with funcNLV.m & funcCLV.m.
...
%
% If using Competitive LV: These globally defined variables are defined:
...
%                           accounted for.
%
% On the Competitive LV, these variables are defined in funcCLV.m:
%   fixEqu, newAlpha, constantR
%
...
% 1C: Prompts if user is using a competitive or niche LV model
lvType = input('Select either a Competitive or Niche LV Model (C/N): ','s');

% 1D: Defines the variables for the competitive LV
if (lvType == 'c' || lvType == 'C');
    maxNode = 8;
    equName = @funcCLV;         % Name of file w. Equations
    masterNodes = [4 8];        % States which nodes are Master
...
% 1E: Defines the variables for the niche LV model
if (lvType == 'n' || lvType == 'N');
    maxNode = 50;
...
% 1F: These parameters are used for Method 8
weightValue = 0.5;
for i1 = 1:maxNode
	weightValues(i1) = weightValue;
end
% weightValues = [0, 0, 1, 0, 1, 0, 0, 0];	
desiredValues = [18.9308, 17.9217, 0, 57.0000, ...
                45.7847, 12.9408, 0.0010, 10.0000]; 
...
% 1D: Prints out results for each SKO Node & Wildtype Values
display(results);
